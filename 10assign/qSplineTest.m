...
% qRHS(1) = cos(xMin) * (h/2);
% qRHS(1) = 0;
qRHS(1) = ((f(2) - f(1))/ (h)) * (h/2);
% qRHS(1) = ((4*f(2) - (3*f(1)) - f(3) )/ (2*h) ) * (h/2)   ;
for i=2:n+1
  A(i,i) = 1;
  A(i,i-1) = 1;
  qRHS(i) = f(i) - f(i-1);
...
estOrder = zeros(nRefine+1, 1);

for i=1:nRefine-1
  estOrder(i) = log2( (splineErrorNorm(i+1) - splineErrorNorm(i) ) / (splineErrorNorm(i+2) - splineErrorNorm(i+1) )  );
end 
...
fs = [' # panels            Error                         Estimate Order',sprintf('\n')];

for kStep = 1:nRefine+1
  n  = nStart*2^(kStep-1);
  fs = [fs,sprintf('  %-3d                %-15.10e                  %-15.10e \n',n,splineErrorNorm(kStep), estOrder(kStep))];
end
fs
